...
class Game:
    ...
    def __init__(self, game_board, state=None):
        self.current_state = State(0, 0) if state is None else state
        self.turn = self.AI
...
def query_ai(board_state):
    """
    Translate board state from 2D array to bitboard representation
    """
    ...
    for row in range(6):
        for col in range(7):
            if board_state[row][col] == 1:
                ai_board |= 1 << (col * 7 + row)
            if board_state[row][col] != 0:
                total_board |= 1 << (col * 7 + row)
    game = Game(board_state, state=State(ai_board, total_board))
    winner = None
    if game.is_game_over():
        winner = 'computer' if ~game.turn == -1 else 'human' if ~game.turn == 0 else 'draw'
    print_board(game.current_state)
    return game.next_turn(), winner

if __name__ == "__main__":
    print("Welcome to Connect Four!")

    from board_state import get_board_state
    from color_seg import color_seg
    from drop_chip import drop_chip
    import numpy as np
    ...
    while True:
        ...
        for row in range(6):
            for col in range(7):
                if board_state[row][col] == 1:
                    ai_board |= 1 << (col * 7 + row)
                    print("Here: {0:049b}".format(1 << (col * 7 + row)))
                if board_state[row][col] != 0:
                    total_board |= 1 << (col * 7 + row)
                    print("HH", total_board & (1 << (col * 7 + row)))
        print("AI Board: {0:049b}".format(ai_board))
        print("Total Board: {0:049b}".format(total_board))
        board_state = State(ai_board, total_board)
        game = Game(board_state, state=board_state)
        ai_col = 6 - game.next_turn()
        print('ai: ', ai_col)
        color_seg()
        drop_chip(ai_col)
        print_board(game.current_state)
